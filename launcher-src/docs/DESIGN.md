...
## Выпуск 0.1.9 — языки и понятный запуск

В верхней части окна добавлен единый селектор: автоматически / Русский / English / Polski. Автоопределение использует список языков ОС, ручной выбор сохраняется между запусками. При смене языка в настройках сохраняются редактируемые параметры. Во время запуска/работы игры смена языка и повторный запуск заблокированы.

Вместо раннего снятия блокировки после строки Fabric показываются этапы загрузки модов, ресурсов, звука и подключения. После появления окна остаётся статус работающей игры; кнопка освобождается по завершению процесса. Сырые строки консоли в интерфейс не копируются.

Перед стартом `create.krwa.ru:25565` добавляется в `servers.dat` без дубликатов и без потери пользовательских записей. NBT обрабатывается в основном процессе Electron, поскольку компилятор библиотеки использует eval, запрещённый в интерфейсе.

Проверены все шаблоны на трёх языках, 10 модульных тестов, ESLint, запуск игры из исходников и установленной Windows 0.1.9. Полный клиент запускается со стандартным набором и с Controlify при выключенных картах; блокировка кнопки и её восстановление после закрытия подтверждены. Сборки Windows/Linux опубликованы. Установленная 0.1.8 автоматически перешла на 0.1.9; итоговый ProductVersion и запуск проверены. Linux/Steam Deck проверены сборкой и совместимостью модов, без физического устройства.

Controlify успешно загрузил SDL и 575 схем геймпадов на Windows. В журнале есть предупреждение о разных patch-версиях SDL и сообщение об отключённом upstream enhanced driver Steam Deck; запуск не прерывается. Физическое управление и расширенные возможности SteamOS требуют проверки на устройстве.

## Выпуск 0.1.10 — компактный выбор языка

Верхний селектор языка заменён квадратной кнопкой с флагом текущего языка. На главном экране кнопка стоит в правом нижнем углу непосредственно слева от настроек и больше не перекрывает карточку пользователя. Список Русский / English / Polski / язык системы раскрывается вверх; сохранённый ручной выбор, автоопределение, блокировка во время запуска и сохранение открытых настроек работают как прежде. Меню доступно с клавиатуры: стрелки, Home, End и Escape.

На 1100×700 и 900×600 подтверждены размер кнопок 54×54 px, зазор 12 px, отсутствие пересечений с карточкой пользователя и размещение всего меню внутри окна. Пройдены 10 модульных тестов и ESLint; переключение на Polski проверено в Electron с отдельным профилем. Windows NSIS и Linux AppImage собраны и опубликованы, исходники интерфейса и четыре SVG в обоих `app.asar` совпадают. Установленная 0.1.9 штатно обновилась до 0.1.10; ProductVersion, браузерные загрузки с активным PWA-кэшем и полные хеши публичных файлов проверены.
