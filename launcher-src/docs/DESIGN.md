...
## Выпуск 0.1.12 — надёжный запуск и честный прогресс

Зависание после нажатия «Играть» на 0% было вызвано синхронной ошибкой интерфейса: `toggleLaunchArea()` пыталась отключить удалённый в 0.1.10 элемент `launcherLanguage`, уже после установки признака запуска и блокировки кнопки. Обработчик теперь работает с актуальными кнопкой и пунктами языкового меню, а тест сверяет все литеральные DOM ID из `landing.js` с отрендеренной страницей, чтобы такое расхождение не повторилось.

Этапы без измеримого объёма показываются анимированным неопределённым индикатором; проверка и скачивание используют реальный числовой прогресс, а при загрузке дополнительно показано количество недостающих файлов. Полоса занимает всю ширину строки до правого края процента. Для получения дистрибутива, проверки Java, проверки файлов и скачивания добавлены явные тайм-ауты бездействия и обработка аварийного завершения дочернего helper-процесса, после ошибки интерфейс и системный индикатор корректно освобождаются.

Пройдены 15 модульных тестов и ESLint. В Electron выполнена первичная загрузка 5885 недостающих файлов: клиент дошёл до инициализации звукового движка и подключения к `create.krwa.ru:25565`. Повторный запуск прошёл без скачивания; после обоих запусков кнопка освободилась при штатном закрытии Minecraft. Геометрия в окне подтверждена численно: левый и правый края полосы совпадают с краями области статуса, правый край — с индикатором процентов.

Windows NSIS и Linux AppImage 0.1.12 собраны и опубликованы. Основные файлы интерфейса в обоих `app.asar` совпадают с исходниками; версии и SHA-512 манифестов проверены. Алиасы Windows/Linux ведут на 0.1.12 с `Cache-Control: no-store`, полностью скачанные публичные файлы совпали с локальными по SHA-256. Установленная Windows-версия 0.1.11 штатно обновилась до 0.1.12 и перезапустилась; итоговый ProductVersion подтверждён.
