...
test('renders the complete launcher in each supported language', async () => {
    for(const language of ['en_US', 'ru_RU', 'pl_PL']) {
        ...
        assert.ok(html.includes('id="launcherLanguageButton"'))
        assert.ok(html.includes('id="launcherLanguageMenu"'))
        assert.ok(html.includes('data-language="auto"'))
        assert.ok(html.includes('class="launcherLanguageFlag" data-language="ru_RU"'))
        assert.ok(html.includes('class="launcherLanguageFlag" data-language="en_US"'))
        assert.ok(html.includes('class="launcherLanguageFlag" data-language="pl_PL"'))
        assert.ok(!html.includes('<select id="launcherLanguage"'))
