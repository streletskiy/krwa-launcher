...
function validateSkinBytes(bytes) {
    if (!Buffer.isBuffer(bytes) || bytes.length > 2 * 1024 * 1024 || bytes.length < 24 || bytes.subarray(0, 8).toString('hex') !== '89504e470d0a1a0a') throw new Error(Lang.native('Нужен PNG-файл размером до 2 МБ.'))
    const width = bytes.readUInt32BE(16), height = bytes.readUInt32BE(20)
    if (width < 64 || width % 64 || width * height > 4096 * 4096 || ![width, width / 2].includes(height)) throw new Error(Lang.native('Нужен скин Minecraft: 64 × 64, 64 × 32 или их HD-вариант до 4096 × 4096.'))
}
...
function createAccountAPI(root = YGGDRASIL_API_ROOT) {
    ...
    async function request(route, options = {}) {
        ...
        catch { throw new Error(Lang.native('Не удалось связаться с KRWA. Проверь подключение и попробуй ещё раз.')) }
        let body = {}
        try { body = response.body ? JSON.parse(response.body) : {} } catch { /* Non-JSON proxy failure. */ }
        if (response.statusCode < 200 || response.statusCode >= 300) {
            const code = body.error?.code || body.code || ''
            let message = body.msg || body.errorMessage || Lang.native('Сервис временно недоступен. Попробуй ещё раз.')
            if (response.statusCode === 401) message = Lang.native('Неверный пароль или срок входа истёк. Войди в аккаунт ещё раз.')
            if (response.statusCode === 429) message = Lang.native('Слишком много попыток. Подожди немного и повтори.')
            if (/captcha/i.test(code)) message = Lang.native('Проверь код с картинки. Если он устарел, обнови картинку.')
            if (/username.*(exist|taken)/i.test(code + message)) message = Lang.native('Этот ник уже занят. Выбери другой.')
            if (/email.*(exist|taken)/i.test(code + message)) message = Lang.native('Эта почта уже зарегистрирована. Попробуй войти.')
    ...
    return {
        ...
        changePassword: async (identifier, currentPassword, newPassword, captcha = {}) => {
            ...
            if (!access) { await revoke(login); throw new Error(Lang.native('Не удалось подтвердить аккаунт. Попробуй войти ещё раз.')) }
            try {
        ...
        upload: async ({ uuid, accessToken, type = 'skin', model, bytes }) => {
            ...
            if (!/^[a-f0-9]{32}$/i.test(id) || type !== 'skin') throw new Error(Lang.native('Выбери игровой аккаунт.'))
