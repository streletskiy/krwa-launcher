...
const dataPath = path.join(sysRoot, '.krwa-launcher')
...
/**
 * Update the access token of an authenticated KRWA account.
 * 
 * @param {string} uuid The uuid of the authenticated account.
...
exports.updateMojangAuthAccount = function(uuid, accessToken){
    ...
    config.authenticationDatabase[uuid].type = 'yggdrasil'
    return config.authenticationDatabase[uuid]
}

/**
 * Adds an authenticated KRWA account to the local launcher configuration.
 * 
 * @param {string} uuid The uuid of the authenticated account.
